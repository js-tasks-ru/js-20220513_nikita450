...
export default class SortableTable {
  ...
  constructor(
    ...
    if (url) {
      ...
      this.elementBodyLink.parentElement.classList.add(
        "sortable-table_loading"
      );
      this.data = data;
      this.elementBodyLink.innerHTML = this.getTemplateTableBody(this.data);
      this.elementBodyLink.parentElement.classList.remove(
        "sortable-table_loading"
      );
    }
  }
  ...
  _initEvents() {
    if (!this.onArrowLink) {
      this.onArrowLink = document.querySelector(
        `[data-id=${this.defaultSort.id}] .sortable-table__sort-arrow`
      );
      if (this.onArrowLink) {
        this.onArrowLink.style.opacity = 1;
      } else {
        this.onArrowLink = this.element.querySelector(
    ...
    this.element
      ...
      .addEventListener("pointerdown", this.handleSortEvent.bind(this));
  ...
  handleSortEvent(event) {
    ...
    if (sorTablCell.dataset.sortable === "false") {
      return;
    }
    ...
    const newArrowLink = sorTablCell.querySelector(
      ".sortable-table__sort-arrow"
    );
  ...
  sort(
    fieldValue = this.defaultSort.id,
    orderValue = this.defaultSort.order,
    isSortLocally = this.isSortLocally
  ) {
    if (isSortLocally) {
  ...
  sortOnClient(fieldValue, orderValue) {
    ...
    this.elementBodyLink.innerHTML = this.getTemplateTableBody(
      [...this.data].sort((a, b) => {
        return (
          direction *
          this.sortByType(
            this.getSortType(fieldValue),
            a[fieldValue],
            b[fieldValue]
          )
        );
  ...
  sortOnServer(fieldValue, orderValue) {
    ...
    this.fetchData({ order: orderValue, field: fieldValue }).then((data) =>
    ...
    return this.headerConfig
      .map((item) => {
        ...
        <div class="sortable-table__cell" data-id="${item.id}" data-sortable="${
          item.sortable
        }" data-order=${this.defaultSort.order || "asc"}>
  ...
  update(...args) {
    ...
    if (args[0] instanceof Date) {
      this.fetchData({ start: args[0], end: args[1] }).then((data) =>
        this._replaceData(data)
      );
    }
  }
  ...
  _addData(dataArray) {
    ...
    this.elementBodyLink.insertAdjacentHTML(
      "beforeend",
      this.getTemplateTableBody(dataArray)
    );
    this.elementBodyLink.parentElement.classList.remove(
      "sortable-table_loading"
    );

...
export default class ColumnChart {
  ...
  constructor(options = {}) {
    ({
      ...
      value: this.value = 0,
      data: this.data = [],
      formatHeading: this.formatHeading = (item) => item,
      label: this.label = "",
      link: this.link,
      chartHeight: this.chartHeight = 50,
      immediateFetch: this.immediateFetch = true,
    } = options);
  ...
  render() {
    ...
    if (this.data.length) {
      wrapper
        ...
        .append(...this._formDataColumns(this.data));
      wrapper
        .querySelector(".column-chart")
        .classList.remove("column-chart_loading");
    }
  ...
  fetchData({ start = this.from, end = this.to, url = this.url } = {}) {
    return fetchJson(
      `${BACKEND_URL}/${url}?` +
        new URLSearchParams({
  ...
  update(...args) {
    ...
    if (args[0] instanceof Date) {
      this.fetchData({ start: args[0], end: args[1] }).then((res) =>
        this.update(res)
      );
    }
